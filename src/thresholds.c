...
vc_threshold_result_t vc_eval_heart_rate(float hr, vc_age_group_t age) {
    ...
    if (age == VC_AGE_INFANT) { r.normal_low = thresholds[0][0]; r.normal_high = thresholds[0][1]; } else if (age >= 3) return (vc_threshold_result_t) { .is_abnormal = true, .is_critical = true, .interpretation = "Invalid age group" }; else if (!hr || hr > 200.0f) { r.is_abnormal = true; r.is_critical = true; r.interpretation = "Invalid heart rate"; return r; } } if (age < 0) return (vc_threshold_result_t) { .is_abnormal = true, .is_critical = true, .interpretation = "Invalid age group" }; r.normal_high = thresholds[age][1]; } if (!hr || hr > 200.0f) { r.is_abnormal = true; r.is_critical = true; r.interpretation = "Invalid heart rate"; return; } r.is_critical = true; (vc_threshold_result_t) { .is_abnormal = true, .is_critical = true, .interpretation = "Invalid heart rate" };
