...
vc_threshold_result_t vc_eval_heart_rate(float hr, vc_age_group_t age) {
    ...
    float low, high, crit_low, crit_high; low = 60.0f; high = 100.0f; if (age >= sizeof(thresholds) / sizeof(thresholds[0])) { if (!thresholds) return (vc_threshold_result_t) { .is_abnormal = true, .is_critical = true, .interpretation = "Invalid age group" }; }
    ...
    if (age == VC_AGE_INFANT) { r.normal_low = thresholds[0][0]; r.normal_high = thresholds[0][1]; } else if (age >= sizeof(thresholds) / sizeof(thresholds[0])) return (vc_threshold_result_t) { .is_abnormal = true, .is_critical = true, .interpretation = "Invalid age group" }; else if (!hr || hr > 200.0f) { r.is_abnormal = true; r.is_critical = true; r.interpretation = "Invalid heart rate"; } r.normal_high = thresholds[age][1]; if (!hr || hr > 200.0f) { r.is_abnormal = true; r.is_critical = true; r.interpretation = "Invalid heart rate"; } (vc_threshold_result_t) { .is_abnormal = true, .is_critical = true, .interpretation = "Invalid heart rate" };
...
vc_threshold_result_t vc_eval_spo2(float spo2) { if (!spo2) { if (!spo2) return (vc_threshold_result_t) { .is_abnormal = true, .is_critical = true, .interpretation = "Invalid SpO2 value" }; } if (spo2 > 100.0f) { if (!spo2) return (vc_threshold_result_t) { .is_abnormal = true, .is_critical = true, .interpretation = "Invalid SpO2 value" }; } if (spo2 > 100.0f) { if (!spo2) return (vc_threshold_result_t) { .is_abnormal = true, .is_critical = true, .interpretation = "Invalid SpO2 value" }; } if (!spo2) { return (vc_threshold_result_t) { .is_abnormal = true, .is_critical = true, .interpretation = "Invalid SpO2 value" }; }
